...
- **立直四麻**：起家 25000，支持东1局显示、本场、供托、符番自动算点
- **三麻立直**：起家 35000，三座位布局，自摸损算点
- **通用**：保留原有手动转账记账

日麻交互：点赢家**分数**=自摸；拖拽放铳者→赢家=荣和；底部可**流局**并标记不听罚符。算点逻辑移植自 OpenMahjong `packages/core/src/mahjong/defen.ts`（本地 `js/riichi/`），运行 `npm test` 可验证黄金用例。
